...
const InputLogo = ({ icon, text }) => {
    ...
    return (
        <div class="relative">
            ...
            <input type="text" id={text} class="mb-0 pl-11 text-md  rounded-md border-b py-2 focus:outline-none focus:border-orange-500 focus:border-b-2 transition-all duration-200 shadow-lg peer" autocomplete="off"></input>
            <label for={text} class="m-0 ml-5 pl-5 text-md absolute left-0 top-2 cursor-text peer-focus:text-md peer-focus:-top-6 peer-focus:text-orange-700 transition-all duration-200">{text}</label>
        </div>
    );
